...
for model_name in model_names:
    if model_name=='CAMS':
        ...
        forecasts[model_name] = forecasting.forecast_pm2p5_v3(forecast_initial_date,pm2p5,df_optimal_cams,\
            df_optimal_gfs,model_name=model_name,mos_correction=False)
        
forecasts['MEAN'] = np.median(forecasts.iloc[:,:],axis = 1)
...
plt.plot(dates_forecast,np.median(forecasts.iloc[:,:-3],axis = 1),label = 'Media del ensamble',
         color='teal',lw=1.4)
...
plt.plot(dates_forecast,np.median(forecasts.iloc[:,:-4],axis = 1),label = 'Media del ensamble',
...
plt.plot(dates_forecast,np.median(forecast_24h_mean.iloc[:,:-4],axis = 1),label = 'Media del ensamble',
         color='teal',lw=1.4)
