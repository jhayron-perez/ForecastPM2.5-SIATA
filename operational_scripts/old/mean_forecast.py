...
cams = forecasting.get_cams_for_forecast(forecast_initial_date,cams_path,operational = True)
gfs,gfs_optimal = forecasting.get_gfs_for_forecast(forecast_initial_date,gfs_path,operational = True)
forecast_initial_date = forecast_initial_date - dt.timedelta(hours=1)

forecasts = pd.DataFrame(index = cams['pm2p5'][forecast_initial_date+dt.timedelta(hours = 1):forecast_initial_date+\
                                                  dt.timedelta(hours = 96)].index)

for model_name in ['LR','KNN','GB','SVR','NN','RF','ANN','CAMS']:
    if model_name=='CAMS':
        forecasts[model_name] = cams['pm2p5'][forecast_initial_date+dt.timedelta(hours = 1):forecast_initial_date+\
                                                 dt.timedelta(hours = 96)].values
    else:
        forecasts[model_name] = forecasting.forecast_pm2p5(forecast_initial_date,pm2p5,cams,gfs_optimal,model_name=model_name)
        
forecasts['MEAN'] = np.mean(forecasts.iloc[:,:-1],axis = 1)
forecasts['MIN'] = np.min(forecasts.iloc[:,:-2],axis = 1)
forecasts['MAX'] = np.max(forecasts.iloc[:,:-3],axis = 1)
...
plt.plot(cams.aod550,label = 'AOD Total',alpha=1,color='darkblue')
plt.plot(cams.omaod550,label = 'AOD Materia Orgánica',ls='--',alpha=1,color='green')
...
plt.plot(cams.bcaod550,label = u'AOD Carbón Negro',ls='--',alpha=1,color='black')
plt.plot(cams.duaod550,label = u'AOD Polvo',ls='--',alpha=1,color='darkorange')
plt.plot(cams.ssaod550,label = u'AOD Sal Marina',ls='--',alpha=1,color='darkgray')
...
plt.plot(dates_forecast,forecast_24h_mean['CAMS'].values,label = 'CAMS (ECMWF)',ls='-.',alpha=0.5,color='gray')
...
plt.ylim(np.min([pd.concat([pm2p5_24h_mean,forecast_24h_mean]).min().MIN-10,0]),\
        pd.concat([pm2p5_24h_mean,forecast_24h_mean]).min().MAX+10)
